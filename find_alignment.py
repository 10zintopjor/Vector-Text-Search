...
volume_path = "/home/basti/data/buddhanexus-pedurma/wylie/test.tsv"
query_path = "/home/basti/data/tibetan/tsv/"
volume_path = "volumes/I1PD95883.txt.w.tsv"
vectorlength = 200
gapsize = 200 
...
def read_file(path):
    df = pd.read_csv(path,sep="\t",names=["segmentnr","orig","stemmed"],dtype="str")
...
volume_file_df, volume_stems = read_file(volume_path)
...
def get_vector(stem,vector_model):
    if stem in vector_model.key_to_index.keys():
...
for file in tqdm(os.listdir("/home/basti/data/tibetan/tsv/")):
    filename = os.fsdecode(file)
    short_path = Path(filename).stem
    if ".tsv" in filename and not "N" in filename and "K02D0008-5" in filename:
